...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("The effect prefab to use when spawning enemies")]
    public GameObject spawnEffect = null;
    [Tooltip("The target of the spawned enemies")]
    public Transform target = null;
    [Tooltip("The distance to player to decide enemy spawn start")]
    ...
    private void CheckSpawnTimer()
    {
        ...
        if (Time.timeSinceLevelLoad > lastSpawnTime + spawnDelay && (currentlySpawned < maxSpawn || spawnInfinite))
        {
            ...
            // Spawn an enemy
            if (GameObject.FindGameObjectWithTag("Player") != null && (target.position - transform.position).magnitude < activationRange)
            {
    ...
    private void SpawnEnemy(Vector3 spawnLocation)
    {
        ...
        if (enemyPrefab != null)
        {
            ...
            Instantiate(spawnEffect, spawnLocation, spawnEffect.transform.rotation, null);
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 startPosX = transform.position + new Vector3(spawnRangeX, 0, 0);
        Vector3 endPosX = transform.position + new Vector3(-spawnRangeX, 0, 0);
        Gizmos.DrawLine(startPosX, endPosX);

        Gizmos.color = Color.green;
        Vector3 startPosY = transform.position + new Vector3(0, spawnRangeY, 0);
        Vector3 endPosY = transform.position + new Vector3(0, -spawnRangeY, 0);
        Gizmos.DrawLine(startPosY, endPosY);
    }
}
